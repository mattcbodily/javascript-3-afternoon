...
//Code Here
let totalPopulation = populations.reduce((tot, e, i, arr) => {
  return tot + e;
})
...
/*
  Use a higher order method to get all the order totals after adding in the sales tax. Your answer should be an array of numbers, one total 
  for each order.
*/

let orderTotals = orders.map((obj) => {
  return obj.price * (obj.tax + 1); 
})
